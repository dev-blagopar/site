...
?>
	<NOSCRIPT>
		...
		<?print_r($server->getDocumentRoot().$templateFolder);?>
	</NOSCRIPT>
<?
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<div id="bx-soa-orderSave">
					...
					<a href="javascript:void(0)" style="margin: 10px 0" class="pull-right btn btn-default btn-lg hidden-xs order_btn"
...
foreach($arResult['BASKET_ITEMS'] as $item){
	$ids_arr[] = $item['PRODUCT_ID'];
	$names_arr[] = '\''.$item['NAME'].'\'';
	$prices_arr[] = round($item['PRICE'], 4);
	$quantities_arr[] = $item['QUANTITY'];
}
