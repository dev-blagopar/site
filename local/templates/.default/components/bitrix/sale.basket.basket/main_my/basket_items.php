...
if($pack <>''){
		...
		$oldPrice_formatted =  $arItem["FULL_PRICE_FORMATED"];

    	/*$arItem["PRICE"] = $arItem["PRICE"] * $coef;
          
		$arItem["PRICE_FORMATED"] = number_format($arItem["PRICE"], 2, ',', ' ')." руб. / $pack";
        
        $arItem["FULL_PRICE"] = $arItem["FULL_PRICE"] * $coef;
		
        $arItem["FULL_PRICE_FORMATED"] = number_format($arItem["FULL_PRICE"], 2, ',', ' ')." руб.";  */
        
        $arItem["SUM"] = $arItem["SUM"] * $coef;
		
		$fullSum = $fullSum + $arItem["SUM"];
        
        $arItem["SUMM_FORMATED"] = number_format($arItem["SUM"], 2, ',', ' ')." руб.";  
        
        $arItem["PROPERTY_M2_VALUE"] = str_replace(",",".",$arItem["PROPERTY_M2_VALUE"]);
      
        $arItem["PROPERTY_M2_VALUE"] = (float)$arItem["PROPERTY_M2_VALUE"] * (int)$arItem["QUANTITY"]; 
								...
								<td class="cost-cell <?=( $bTypeColumn ? 'notes' : '' );?>">
									...
									<div class="cost prices clearfix">
										...
										<?}else{?>
											...
											<div class="price"><?=$arItem["PRICE_FORMATED"];?> / <?=$arItem["MEASURE_TEXT"]?></div>
										...
										<?if ($pack <> ''){?>
										<div class="description">(Продается упаковками <?=$pack?>)</div>
										<?}?>
		...
		<? $arResult["allSum"] = $fullSum;        
           $arResult["allSum_FORMATED"] = number_format($arResult["allSum"], 2, ',', ' ')." руб.";?>
