...
window.JCCatalogElement.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		setViewedProduct(this.offers[index].ID, {
			...
			'PICTURE_ID': this.offers[index].SLIDER_COUNT ? this.offers[index].SLIDER[0].ID : false,
	...
	if(buyBlock.find('.counter_wrapp').find('.button_block').length){
		if(arBasketAspro["BASKET"] && (arBasketAspro["BASKET"].indexOf(obj.ID) !== -1 || arBasketAspro["BASKET"].indexOf(obj.ID * 1) !== -1)){
			...
			if(className){
				buyBlock.find('.counter_wrapp').find('.button_block').addClass('wide').html(obj.HTML);
				if(typeof arBasketAspro !=="undefined"){
					if(arBasketAspro["SUBSCRIBE"] && (arBasketAspro["SUBSCRIBE"].indexOf(obj.ID) !== -1 || arBasketAspro["SUBSCRIBE"].indexOf(obj.ID * 1) !== -1)){
