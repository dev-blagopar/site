			...
			<tbody>
				...
				<?foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
					...
					<tr data-id="<?=$arItem["ID"]?>"  <?if($arItem["QUANTITY"]>$arItem["AVAILABLE_QUANTITY"]):?>data-error="no_amounth"<?endif;?> data-product_id="<?=$arItem["PRODUCT_ID"]?>" >
						...
						<?endif;?>
						<?if ($bDeleteColumn):?>
							<td class="remove-cell"><a class="remove" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"
							title="<?=GetMessage("SALE_DELETE")?>" 
							onclick="pushDelItem('<?=$arItem["PRODUCT_ID"]?>', '<?=$arItem["NAME"]?>', '<?=$arItem["PRICE"]?>')">><i></i></a></td>
						<?endif;?>
					</tr>
					<?endif;?>
				<?endforeach;?>
...
<?endif;?>
<script type="text/javascript">
function pushDelItem(item_id, item_name, item_price){
	dataLayer.push({
			"ecommerce": {
				"add": {
					"products": [
